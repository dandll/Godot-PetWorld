...
#玩家位置
var attakPlayerPositionArr=[Vector2(88,176),Vector2(40,176),Vector2(136,176),Vector2(40,216),Vector2(88,216),Vector2(136,216)];
#敌人位置
var attakEmeyPositionArr=[Vector2(88,80),Vector2(40,80),Vector2(136,80),Vector2(40,50),Vector2(88,50),Vector2(136,50)];
...
func _ready():
	...
	#动态创建敌人
#	var emeyNum=0
#	while emeyNum< GameGlobal.AttackGuanQia:
#		var mob = mob_scene.instance().duplicate()
...
#		emeyNum+=1
#		pass
	if GameGlobal.yeWaiEmeyXuHao<=5:
		var emeyNum=0
		while emeyNum< GameGlobal.yeWaiEmeyXuHao:
			var mob =null;
			if GgInfo.playerData[0][1]["dj"] <=10:
				mob=GameGlobal.emey01Const.instance().duplicate()
				if emeyNum==4&&GameGlobal.yeWaiEmeyXuHao==5:
					mob = GameGlobal.emey02Const.instance().duplicate()
			elif GgInfo.playerData[0][1]["dj"] <=20:
				mob=GameGlobal.emey02Const.instance().duplicate()
				if emeyNum==4&&GameGlobal.yeWaiEmeyXuHao==5:
					mob = GameGlobal.emey03Const.instance().duplicate()
			elif GgInfo.playerData[0][1]["dj"] <=30:
				mob=GameGlobal.emey03Const.instance().duplicate()
				if emeyNum==4&&GameGlobal.yeWaiEmeyXuHao==5:
			...
			if GgInfo.playerData[0][1]["dj"] > 10:
#				#mob.dj=((GameGlobal.AttackGuanQia-1)*10)+(randi()%(10)+1)
#				mob.dj=GgInfo.playerData[0][1]["dj"]+(randi()%(5))
#				if GameGlobal.yeWaiEmeyXuHao==5:
#					mob.dj=GgInfo.playerData[0][1]["dj"]+3
#					if emeyNum==4:
#						mob.dj=GgInfo.playerData[0][1]["dj"]+6
				mob.dj=GgInfo.playerData[0][1]["dj"]+GameGlobal.yeWaiEmeyXuHao
			else:
				#mob.dj=((GameGlobal.AttackGuanQia-1)*10)+(randi()%(5)+1)
				mob.dj=GgInfo.playerData[0][1]["dj"]
				pass
			mob.teamNum=1
			#print("怪物等级："+mob.dj as String)
...
#	print(GameGlobal.attakEmey01Obj.isOutScene as String)
	if IsAttakSuccess():#GameGlobal.attakEmey01Obj.isOutScene:
		...
		var sljy = GgInfo.GetYeGuaiJingYan();
...
func _on_Timer_timeout():
	GameGlobal.AttakMapGameSceneLoad()
	pass
