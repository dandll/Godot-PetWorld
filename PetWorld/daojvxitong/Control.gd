...
#			$Panel/Node2D/Node2D/PopupMenu.rect_position = itembox.rect_position+Vector2(650,250)
			$Panel/Node2D/Node2D/PopupMenu.rect_position = itembox.rect_position+$Panel/GridContainer.rect_global_position
...
func _on_mouse_entered(item):
	...
	if Daojvlan.daojvlan[item_index] != null:
		print("item.rect_position:"+item.rect_position.x as String+","+item.rect_position.y as String)
		$Panel/Node2D/Sprite.global_position = item.rect_position
